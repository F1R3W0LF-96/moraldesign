...
function Services() {
  ...
  const renderServices = () => {
    const totalBlock = services.length / 3;
    let html = [];
    for (let i = 0; i < totalBlock; i++) {
      const renderData = [...services.slice(i * 3, 3 * (i + 1))];
      html.push(renderBlock(renderData));
    }
    return html;
  };
  const renderBlock = (data) => {
    return (
      <div class="row">
        {data.map((itemDetail, key) => {
          return singleRenderElement(itemDetail, key);
        })}
      </div>
    );
  };

  const singleRenderElement = (data, key) => {
    return (
      <div class="col-lg-4 col-md-6">
        <div className="single-services">
          <div className="service-icon">{data.image}</div>
  ...
  return (
    <section id="services">
      ...
      <div class="container">{renderServices()}</div>
    </section>
  );
}
